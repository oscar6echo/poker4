...
int main()
{
    ...
    int sums[50000], sumKeys;       // array of all possible sums of key[c1-5]

    int key[13] = {0, 1, 2, 4, 8, 16, 32, 0, 0, 0, 0, 0, 0}; // init keys - empirical
    k = 7;
                    ...
                    for (c3 = c2 + 1; c3 <= k; c3++)
                    {
                        for (c4 = c3 + 1; c4 <= k; c4++)
                        {
                            for (c5 = c4 + 1; c5 <= k; c5++)
                            {
                                for (c5 = c4 + 1; c5 <= k; c5++)
                                {
                                    sums[sumKeys] = key[c1] + key[c2] + key[c3] + key[c4] + key[c5];
                                    sumKeys++;
                                }
                            }
                        }
